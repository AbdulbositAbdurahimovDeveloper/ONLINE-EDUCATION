...
public class ModuleController {
    ...
    @GetMapping("/{id}")
    @PreAuthorize("hasRole('ADMIN') or hasRole('INSTRUCTOR') or @courseSecurity.isUserEnrolled(authentication, #id)")
    @Operation(
    ...
    @Operation(
            ...
            requestBody = @io.swagger.v3.oas.annotations.parameters.RequestBody(
                    ...
                    required = true,
                    content = @Content(schema = @Schema(implementation = ModuleCreateDTO.class),
                            examples = @ExampleObject(value = "{ \"title\": \"Spring Boot Module\", \"courseId\": 5 }"))
            ),
            responses = @ApiResponse(responseCode = "200", description = "Module successfully created")
    )
    ...
    public ResponseEntity<ResponseDTO<PageDTO<LessonResponseDTO>>> readLessons(@PathVariable Long id,
    ...
    public ResponseEntity<ResponseDTO<PageDTO<PaymentDTO>>> readPayments(@PathVariable Long id,
                                                                         @RequestParam(defaultValue = "0") Integer page,
                                                                         @RequestParam(defaultValue = "10") Integer size) {
    ...
    @PutMapping("/{id}")
    @PreAuthorize("hasRole('ADMIN') or hasRole('INSTRUCTOR')")
    @Operation(summary = "Update module", description = "Updates module information by ID")
    ...
    @PreAuthorize("hasRole('ADMIN') or hasRole('INSTRUCTOR')")
    @Operation(summary = "Reorder modules", description = "Updates the order of modules inside a course.")
    public ResponseEntity<ResponseDTO<String>> reorder(@PathVariable Long courseId, @RequestBody List<ModuleOrderIndexDTO> moduleOrderDTOS) {
        moduleService.updateModuleOrderIndex(courseId, moduleOrderDTOS);
        return ResponseEntity.ok(ResponseDTO.success("Module order updated successfully"));
    }

    @DeleteMapping("/{id}")
    @PreAuthorize("hasRole('ADMIN')")
    @Operation(summary = "Delete module", description = "Deletes module by ID")
    public ResponseEntity<ResponseDTO<String>> delete(@PathVariable Long id) {
        moduleService.delete(id);
        return ResponseEntity.ok(ResponseDTO.success("Module deleted successfully"));
    }
}
