...
public class CourseController {
    ...
    @Operation(
            ...
            description = "Creates a new course. Accessible only by ADMIN or INSTRUCTOR.",
            requestBody = @io.swagger.v3.oas.annotations.parameters.RequestBody(
                    description = "Course creation payload",
                    content = @Content(examples = {
                            @ExampleObject(
                                    name = "New Course Example",
                                    value = "{ \"title\": \"Java Backend Masterclass\", \"description\": \"Full backend development course\", \"price\": 199.99, \"categoryId\": 2 }"
                            )
                    })
            )
    )
    ...
    public ResponseEntity<ResponseDTO<?>> patch(
        ...
        return ResponseEntity.ok(ResponseDTO.success("Course marked as successful"));
    }
