...
public class NotificationService {
    ...
    public void sendReplyToStudentByEmail(ContactMessage originalMessage, String adminEmail) {
        try {
            ...
            log.info("Reply email sent successfully to {} for message ID {}", adminEmail, originalMessage.getId());

        } catch (MessagingException e) {
            log.error("Email yuborishda xatolik (talabaga javob): {}", e.getMessage());
            // Xatolikni log qilish va istisno tashlash
            throw new IllegalStateException("Email yuborishda xatolik yuz berdi", e);
        } catch (Exception e) {
            log.error("Umumiy xatolik (talabaga javob): {}", e.getMessage());
            throw new IllegalStateException("Javob yuborishda umumiy xatolik", e);
        }
    }
